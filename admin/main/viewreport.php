...
<a  href="managereports.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>

<?php
	include('../connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM  reports WHERE id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
<link href="../style.css" media="screen" rel="stylesheet" type="text/css" />
<center><h4><i class="icon-edit icon-large"></i>  Report Details</h4></center>
<hr>
...
<td style="padding: 10px;
				...
				background-color: #f4f4f4;
				text-align: center;
				color: #7d7d7d;"> <?php echo $row['incident']; ?></td>
</tr>
<tr>
<td> Description:  </td>
<td style="padding: 10px;
...
<td style="padding: 10px;
				...
				background-color: #f4f4f4;
				text-align: center;
				color: #7d7d7d;"> <?php echo $row['severity']; ?></td>
</tr>
<tr>
<td> Location :  </td>
<td style="padding: 10px;
